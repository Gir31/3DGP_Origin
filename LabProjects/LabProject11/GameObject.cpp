...
void CGameObject::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera)
{
	...
	if (m_pMesh) m_pMesh->Render(pd3dCommandList, pCamera);
}

void CGameObject::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera, XMFLOAT4X4* pxmf4x4World, CMesh* pMesh)
{
	XMFLOAT4X4 Origin4X4 = m_xmf4x4World;

	OnPrepareRender();
	//객체의 정보를 셰이더 변수(상수 버퍼)로 복사한다.
	m_xmf4x4World = *pxmf4x4World;
	UpdateShaderVariables(pd3dCommandList);
	if (m_pShader) m_pShader->Render(pd3dCommandList, pCamera);
	if (pMesh) pMesh->Render(pd3dCommandList, pCamera);

	m_xmf4x4World = Origin4X4;
}
...
int CGameObject::getTargetStage()
{
