...
void CGameFramework::ChangeSwapChainState()
{
	...
	//창모드로 바꿀때 안움직이는 이유: 창모드로바꿀때 기존 버퍼들을 방출고 새로만들 수 있도록 만들어준것
...
void CGameFramework::BuildObjects()
{
	...
	CTank* pAirplanePlayer = new CTank(m_pd3dDevice,
	...
	case WM_KEYUP:
		...
		case VK_CONTROL:
			if (manager->getCurrLevel() == 3) {
				((CTank*)m_pPlayer)->FireBullet(m_pSelectedObject);
   ...
   레이어를 x-축 또는 y-축으로 회전한다.*/
	...
	if ((dwDirection != 0) || (cxDelta != 0.0f) || (cyDelta != 0.0f))
	{
		...
		else
		{
			...
			if (dwDirection) m_pPlayer->Move(dwDirection, 500.0f *
...
void CGameFramework::ProcessSelectedObject(DWORD dwDirection, float cxDelta, float
	cyDelta)
{
	m_pSelectedObject->m_bBlowingUp = true;
}