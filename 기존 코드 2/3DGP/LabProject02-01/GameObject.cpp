...
void CExplosiveObject::Animate(float fElapsedTime)
{
	if (m_bIsBullet)
	{
		m_fBulletLife += fElapsedTime;

		// 이동
		XMFLOAT3 pos = GetPosition();
		XMFLOAT3 vel = Vector3::ScalarProduct(m_xmf3MovingDirection, m_fMovingSpeed * fElapsedTime, false);
		pos = Vector3::Add(pos, vel);
		SetPosition(pos);

		// 일정 시간 지나면 비활성화
		if (m_fBulletLife > m_fBulletMaxLife)
			SetActive(false);
