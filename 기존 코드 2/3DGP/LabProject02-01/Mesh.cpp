...
BOOL CMesh::RayIntersectionByTriangle(XMVECTOR& xmRayOrigin, XMVECTOR& xmRayDirection,
	XMVECTOR v0, XMVECTOR v1, XMVECTOR v2,
	float* pfHitDistance)
{
	return TriangleTests::Intersects(xmRayOrigin, xmRayDirection, v0, v1, v2, *pfHitDistance);
}

int CMesh::CheckRayIntersection(XMVECTOR& xmRayOrigin, XMVECTOR& xmRayDirection, float* pfHitDistance)
{
	...
	*pfHitDistance = FLT_MAX;
	...
	{
		UINT i0 = m_pnIndices[i];
		UINT i1 = m_pnIndices[i + 1];
		UINT i2 = m_pnIndices[i + 2];

		XMVECTOR v0 = XMLoadFloat3(&m_pxmf3Positions[i0]);
		XMVECTOR v1 = XMLoadFloat3(&m_pxmf3Positions[i1]);
		XMVECTOR v2 = XMLoadFloat3(&m_pxmf3Positions[i2]);

		float fHitDistance = 0.0f;
		if (RayIntersectionByTriangle(xmRayOrigin, xmRayDirection, v0, v1, v2, &fHitDistance))
		{
			if (fHitDistance < *pfHitDistance)
			{
				*pfHitDistance = fHitDistance;
				nIntersections++;
			}
		}
	}
